...
class Config:
    ...
    # Netlify Functions have NO persistent disk — every invocation can land on
    # a different, throwaway container, so SQLite (a local file) cannot be used
    # here. Set DATABASE_URL to a real hosted Postgres instance (e.g. a free
    # Neon.tech or Supabase database) instead. Local/PythonAnywhere deploys
    # without DATABASE_URL set still fall back to the original SQLite file.
    ...
    if _db_url:
        ...
        # needed because Supabase/serverless can silently close idle
        # connections between one Netlify Function invocation and the next.
        # pool_size kept small since each function container gets its own
        # pool, and Supabase's free tier has a limited total connection cap
        # (use Supabase's "Transaction pooler" connection string, not the
        # direct one, so many containers can share it safely).
